...
template <typename T> class Range {
  ...
  public:
    ...
    // 迭代器开始与结束
    const Range &begin(void) const noexcept { return *this; }

    const Range &end(void) const noexcept { return *this; }
    ...
    // 迭代器解引用
    T operator*(void) const noexcept { return tCurrent; }

    // 重设当前值为起始值（const类也可使用）
    void ResetCurrent(void) const noexcept { tCurrent = tBegin; }

    // 设置成员
    void SetBegin(T _tBegin) noexcept { tBegin = _tBegin; }

    void SetEnd(T _tEnd) noexcept { tEnd = _tEnd; }
    ...
    // 获取成员
    T GetBegin(void) const noexcept { return tBegin; }

    T GetEnd(void) const noexcept { return tEnd; }

    T GetStep(void) const noexcept { return tStep; }

    T GetCurrent(void) const noexcept { return tCurrent; }
};
